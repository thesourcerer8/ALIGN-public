...
int
toplevel( const std::vector<string>& args) {
  ...
  string mfile=args[4];
  string dfile=args[5];
  string topcell=args[6];
  int numLayout=std::stoi(args[7]);
  int effort=std::stoi(args[8]);
  if(fpath.back()=='/') {fpath.erase(fpath.end()-1);}
  if(opath.back()!='/') {opath+="/";}

  // Following codes try to get the path of binary codes
  string binary_directory = args[0];
  cout <<"args[0]: "<<binary_directory <<endl;
