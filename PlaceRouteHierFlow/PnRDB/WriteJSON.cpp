...
static int via2int( const PnRDB::Drc_info& drc_info, const string& via)
{
  return stoi(drc_info.MaskID_Via.at(drc_info.Viamap.at(via)));
}

static int metal2int( const PnRDB::Drc_info& drc_info, const string& metal)
{
  return stoi(drc_info.MaskID_Metal.at(drc_info.Metalmap.at(metal)));
}
...
static bool
addMetalBoundaries (json& jsonElements, struct PnRDB::Metal& metal, const PnRDB::Drc_info& drc_info, double unit) {
    int x[5], y[5];
